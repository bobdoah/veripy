...
class ResolutionWaitQueueSingleQueueTestCase(ComplianceTestCase):
    ...
    def run(self):
        seq = [self.next_seq() for _ in itertools.repeat(None, 3)]
        self.logger.info("Sending three ICMP Echo Requests (seq numbers: %s)..." % ", ".join((str(s) for s in seq)))

        self.node(1).send(IPv6(src=str(self.node(1).link_local_ip()), dst=str(self.target(1).link_local_ip()))/
                    ICMPv6EchoRequest(seq=seq))
        ...
        assertGreaterThan(0, len(r1), "expected to receive at least one ICMPv6 Neighbor Solicitation")
        for p in r1:
        ...
        assertGreaterThan(0, len(r2), "expected to receive at least one ICMPv6 Echo Replies")

        for p in r2:
            assertTrue(p.getlayer(ICMPv6EchoReply).seq in seq, "expected an ICMPv6 echo reply with sequence number in: %s, got %s" % (
                ", ".join(str(s) for s in seq), p.getlayer(ICMPv6EchoReply).seq))
